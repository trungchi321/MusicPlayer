...
public class RecentsTrackAdapter extends RecyclerView.Adapter<RecentsTrackAdapter.MyViewHolder>
    ...
    public void onItemMove(int fromPosition, int toPosition) {
        ...
        new SaveRecents(homeActivity).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    }
    ...
    public void onItemDismiss(int position) {
        ...
        if (position < 10) {
            ...
            RecentlyPlayed recentlyPlayed =homeActivity.getHomeConfigInfo().getRecentlyPlayed();
            for (int i = 0; i < Math.min(10, recentlyPlayed.getRecentlyPlayed().size()); i++) {
                HomeActivity.continuePlayingList.add(recentlyPlayed.getRecentlyPlayed().get(i));
            }

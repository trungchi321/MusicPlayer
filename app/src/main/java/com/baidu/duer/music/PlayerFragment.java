...
import com.baidu.duer.music.view.CustomAdapter;
import com.baidu.duer.music.view.SnappyRecyclerView;
...
import com.baidu.duer.music.notification.AudioPlayerBroadcastReceiver;
import com.baidu.duer.music.task.HomeConfigInfo;
import com.baidu.duer.music.task.SaveFavourites;
import com.baidu.duer.music.task.SaveQueue;
import com.baidu.duer.music.task.SaveRecents;
...
//    static ImageView currentAlbumArtHolder;
    ...
    public void onAttach(final Context context) {
        ...
        mMediaPlayer.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
            ...
            public void onPrepared(MediaPlayer mp) {
                ...
                snappyRecyclerView.setCurrentPosition(homeConfigInfo.getQueueCurrentIndex());
                snappyRecyclerView.setTransparency();

                new SaveQueue(getActivity(),homeConfigInfo).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                new SaveRecents(getActivity(),homeConfigInfo).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    ...
    public void onResume() {
        super.onResume();
        if (snappyRecyclerView.getCurrentPosition() != homeConfigInfo.getQueueCurrentIndex()) {
            snappyRecyclerView.scrollToPosition(homeConfigInfo.getQueueCurrentIndex());
            snappyRecyclerView.setCurrentPosition(homeConfigInfo.getQueueCurrentIndex());
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        favouriteIcon.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                new SaveFavourites(getActivity(),homeConfigInfo).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            }
        });
        ...
        if (homeConfigInfo.getQueue() != null) {
            customAdapter = new CustomAdapter(getContext(), snappyRecyclerView,homeConfigInfo.getQueue().getQueue());
        } else {
        ...
        if (homeActivity.streamSelected) {
            ...
            try {
                MusicApplication.getImageLoader().DisplayImage(localTrack.getPath(), selected_track_image);
            } catch (Exception e) {
        ...
        try {
            if (homeActivity.streamSelected) {
                if ((homeConfigInfo.getSettings().isStreamOnlyOnWifiEnabled() && mWifi.isConnected()) || !homeConfigInfo.getSettings().isStreamOnlyOnWifiEnabled()) {
                    isPrepared = false;
                    mMediaPlayer.reset();
    ...
    public void addToFavourite() {
        ...
        homeConfigInfo.getFavouriteTracks().getFavourite().add(ut);
        ...
        for (int i = 0; i < favouriteTracks.getFavourite().size(); i++) {
            ...
            if (ut.getType() && ut1.getType()) {
                if (ut.getLocalTrack().getTitle().equals(ut1.getLocalTrack().getTitle())) {
                    favouriteTracks.getFavourite().remove(i);
                    break;
                }
    ...
    public void refresh() {
        ...
        if (snappyRecyclerView.getCurrentPosition() != queueCurrentIndex) {
            snappyRecyclerView.scrollToPosition(queueCurrentIndex);
            snappyRecyclerView.setCurrentPosition(queueCurrentIndex);
            customAdapter.notifyDataSetChanged();
        }
    ...
    public class SaveDNA extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            if (homeActivity.isSaveDNAEnabled) {
                ...
                for (int i = 0; i < savedDNAs.getSavedDNAs().size(); i++) {
                    if (savedDNAs.getSavedDNAs().get(i).getName().equals(savedDNA.getName())) {
                        savedDNAs.getSavedDNAs().remove(i);
                        break;
                    }
                }

                // Add the DNA to the first position.
                savedDNAs.getSavedDNAs().add(0, savedDNA);

                // If number of DNAs get more than 10, then remove the 11th DNA to keep size as 10.
                // Caution : 0-indexed arrays.
                if (savedDNAs.getSavedDNAs().size() > 10) {
                    savedDNAs.getSavedDNAs().remove(10);
                }

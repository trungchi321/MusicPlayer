        ...
        if (list.size() >= 3) {
            ...
            for (int i = 0; i < 3; i++) {
                if (list.get(i).getType()) {
                    ...
                    MusicApplication.getImageLoader().DisplayImage(list.get(i).getLocalTrack().getPath(), holder.img[i]);
                } else {
                    names[i] = list.get(i).getStreamTrack().getTitle();
                    MusicApplication.getImageLoader().DisplayImage(list.get(i).getStreamTrack().getArtworkURL(), holder.img[i]);
                }
